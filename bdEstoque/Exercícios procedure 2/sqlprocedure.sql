...
atendido pela confeitaria”*/
	...
	AS
		...
		BEGIN
			...
			BEGIN
				PRINT ('Não foi possivel cadastrar! não atendemos o bairro ' +@nomeBairro)
			END
			ELSE
			BEGIN
				INSERT tblCliente (nomeCliente,dataNascimento,ruaCliente,numRuaCliente,cepCliente,bairroCliente,estadoCliente,cpfCliente,sexoCliente)
				VALUES (@nome, @datanasc, @rua, @numRua, @cep, @bairro, @estado, @cpf, @sexo)
			END
		END
...
AS
	...
	SET @codCliente =  (SELECT MAX(codEncomenda) FROM tblEncomenda)
